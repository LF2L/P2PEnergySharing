...
class MyProblem(Problem):
    ...
    def __init__(self, FeedInTariff, gridPrices, SOCmin, SOCmax, selfDischarge, charge_efficiency, discharge_efficiency, nominalCapacity, REGen, load, battery_init_val):
        self.FeedInTariff = FeedInTariff # array 
        self.gridPrices = gridPrices # array
        self.SOCmin =  SOCmin # 0.2 # scalar unit: NaN
        self.SOCmax = SOCmax # 0.8 # scalar unit: NaN
        self.selfDischarge = selfDischarge # 0 # sigma 
        self.charge_efficiency = charge_efficiency #1 # scalar unit: NaN
        self.discharge_efficiency = discharge_efficiency #1 # scalar unit: NaN
        self.nominalCapacity = nominalCapacity #1800 # kWh
        self.nbOfStepInOneDay = len(FeedInTariff)
        self.REgenerationForecast = REGen
        self.loadForecast = load
        self.threshold = 1e-1
        self.b_initial_value = battery_init_val#battery initial value

        self.timeslot_duration_hours = 10/60 # hours 

        battery_lower_bound = self.SOCmin*self.nominalCapacity *np.ones(self.nbOfStepInOneDay)
        battery_upper_bound = self.SOCmax*self.nominalCapacity *np.ones(self.nbOfStepInOneDay)

        lower_bound_powerExchange = -max(self.REgenerationForecast) * np.ones(self.nbOfStepInOneDay)
        upper_bound_powerExchange = max(self.loadForecast) * np.ones(self.nbOfStepInOneDay)
        ...
        super().__init__(n_var=self.nbOfStepInOneDay * 2,
                         n_obj=1,
                         n_constr=1,
                         xl=lower_bound,
                         xu=upper_bound)
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        f1 = (np.where(X[:,:144]<0,X[:,:144],0)*self.FeedInTariff + np.where(X[:,:144]>0,X[:,:144],0) * self.gridPrices ).sum(axis=1)

        current_battery_values = X[:,144:]
        prev_battery_values = np.roll(current_battery_values,1,axis=1)
        prev_battery_values[:,0] = self.b_initial_value
        current_battery_values[:,0]= self.b_initial_value

        power_battery_flow = prev_battery_values* (1- self.selfDischarge) - current_battery_values

        power_discharging_from_battery = np.where(power_battery_flow>0, power_battery_flow * self.discharge_efficiency, 0)
        power_charging_to_battery = np.where(power_battery_flow<0, power_battery_flow/ self.charge_efficiency, 0)

        g1 = (X[:,:144] - self.loadForecast + self.REgenerationForecast + power_discharging_from_battery + power_charging_to_battery)**2 - self.threshold
...
if __name__ == '__main__':
    ...
    # termination = get_termination("cv_tol", 1e-2)
    ...
    current_battery_values = res.X[144:]
    prev_battery_values = np.roll(current_battery_values,1)
    # prev_battery_values[0] = initial_value
    #current_battery_values[:,0]= initial_value

    power_battery_flow = prev_battery_values * (1- selfDischarge) - current_battery_values

    power_discharging_from_battery = np.where(power_battery_flow>0, power_battery_flow * discharge_efficiency, 0)
    power_charging_to_battery = np.where(power_battery_flow<0, power_battery_flow/ charge_efficiency, 0)

    balance_check = res.X[:144] + REgenerationForecast - loadForecast + power_discharging_from_battery + power_charging_to_battery
    #print(balance_check)
    ...
    axs[2].plot(range(0,len(res.X[144:])), res.X[144:], label="battery power state of the battery ")
    axs[2].hlines(y=SOCmin*nominalCapacity, xmin = 0 , xmax = len(res.X[144:]), label="level Min")
    axs[2].hlines(y=SOCmax*nominalCapacity, xmin = 0 , xmax = len(res.X[144:]), label="level Min")
    axs[2].set(xlabel='timeslots', ylabel='Power (Wh)', title='Battery levels')
    axs[2].legend()

    axs[3].plot(range(0,len(power_discharging_from_battery)), power_discharging_from_battery, label="battery discharging")
    axs[3].plot(range(0,len(power_charging_to_battery)), power_charging_to_battery, label="battery charging")
    # axs[3].plot(range(0,len(balance_check)), balance_check, label="battery state")
    
    axs[3].set(xlabel='timeslots', ylabel='Power (Wh)', title='Amount of energy exchanged')
    axs[3].legend()
